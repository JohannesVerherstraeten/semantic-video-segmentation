"""
ENet with LSTM in center.

Enet copied from https://github.com/davidtvs/PyTorch-ENet
"""
import torch
import torch.nn as nn
from torch.autograd import Variable

from .basemodel import BaseRNNModel
...
class InitialBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=0, bias=False, relu=True):
        ...
        self.main_branch = nn.Conv2d(in_channels, out_channels - 3, kernel_size=kernel_size, stride=2, padding=padding,
                                     bias=bias)
...
class DownsamplingBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, internal_ratio=4, kernel_size=3, padding=0, return_indices=False,
        ...
        if internal_ratio <= 1 or internal_ratio > in_channels:
            raise RuntimeError("Value out of range. Expected value in the interval [1, {0}], got internal_scale={1}. "
        ...
        # Convolution
        self.ext_conv2 = nn.Sequential(nn.Conv2d(internal_channels, internal_channels, kernel_size=kernel_size,
...
class RegularBottleneck(nn.Module):
    ...
    def __init__(self, channels, internal_ratio=4, kernel_size=3, padding=0, dilation=1, asymmetric=False,
        ...
        if asymmetric:
            self.ext_conv2 = nn.Sequential(nn.Conv2d(internal_channels, internal_channels, kernel_size=(kernel_size, 1),
...
class UpsamplingBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, internal_ratio=4, kernel_size=3, padding=0, dropout_prob=0.,
        ...
        self.ext_conv2 = nn.Sequential(nn.ConvTranspose2d(internal_channels, internal_channels, kernel_size=kernel_size,
                                                          stride=2, padding=padding, output_padding=1, bias=bias),
                                       nn.BatchNorm2d(internal_channels), activation)

        # 1x1 expansion convolution
        self.ext_conv3 = nn.Sequential(nn.Conv2d(internal_channels, out_channels, kernel_size=1, bias=bias),
                                       nn.BatchNorm2d(out_channels), activation)
...
class ENetLSTM(BaseRNNModel):
    """Generate the ENet model with an LSTM in the center.
    ...
    - init_lstm: Whether to initialize the trainable parameters of this cell as a kind-of unit operation.
    """

    def __init__(self, num_classes=20, encoder_relu=False, decoder_relu=True, init_rnn=False, residual_connection=True,
                 summate_res_connection=True, rnn_ksize=3):
        super(ENetLSTM, self).__init__()

        self.residual_connection = residual_connection
        self.summate_res_connection = summate_res_connection
        ...
        # Stage 3.5 - LSTM
        self.lstm = ConvLSTM(128, hidden_channels=128, kernel_sizes=(rnn_ksize, rnn_ksize), num_layers=1,
                             init_param=init_rnn)
        if not summate_res_connection:
            # concatenate lstm result and residual connection instead of summate
            self.rnn_concat = nn.Sequential(nn.Conv2d(128 * 2, 128, kernel_size=1, stride=1, bias=True),
                                             nn.ReLU())
    ...
    def forward(self, x):
        ...
        # Stage 3.5 - LSTM
        if self.summate_res_connection:
            ...
            if self.residual_connection:
                x = torch.cat((self.lstm(x), x), dim=1)
            else:
                x = torch.cat((self.lstm(x), torch.zeros_like(x)), dim=1)
            x = self.rnn_concat(x)
    ...
    def repackage_hidden_state(self):
        self.lstm.repackage_hidden_state()

    def reset_trainable_parameters(self):
        self.lstm.reset_trainable_parameters()

    def recurrent_parameters(self):
        return self.lstm.parameters(recurse=True)
