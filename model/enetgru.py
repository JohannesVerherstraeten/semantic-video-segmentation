...
class ENetGRU(BaseRNNModel):
    """Generate the ENet model with a GRU in the center.
    ...
    def __init__(self, num_classes=20, encoder_relu=False, decoder_relu=True, init_rnn=False, residual_connection=True,
        ...
        # Stage 3.5 - GRU
        self.gru = ConvGRU(128, hidden_channels=128, kernel_sizes=(rnn_ksize, rnn_ksize), num_layers=1,
                           init_param=init_rnn)
    ...
    def forward(self, x):
        ...
        if self.summate_res_connection:
            ...
            if self.residual_connection:
                x = torch.cat((self.gru(x), x), dim=1)
            else:
                x = torch.cat((self.gru(x), torch.zeros_like(x)), dim=1)
            x = self.rnn_concat(x)
    ...
    def repackage_hidden_state(self):
        self.gru.repackage_hidden_state()

    def recurrent_parameters(self):
        return self.gru.parameters(recurse=True)
